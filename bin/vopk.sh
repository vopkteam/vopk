#!/usr/bin/env bash
# vopk - Unified Package Manager Frontend (1.2.0)
# Formerly: apkg
#
...
EOF
  ...
  printf "%sBackends:%s apt • pacman • dnf/yum • zypper • apk • xbps • emerge • brew • pkg\n" "$DIM" "$RESET"
  ui_hr
}
...
cmd_install() {
  ...
  case "${PKG_MGR_FAMILY}" in
    ...
    brew)
      ...
      echo "vopk: Packages to install (Homebrew):"
      printf '  %s\n' "${mapped_b[@]}"
    ...
    freebsd)
      ...
      echo "vopk: Packages to install (FreeBSD pkg):"
      printf '  %s\n' "${VOPK_PRESENT_PKGS[@]}"
    ...
    openbsd)
      ...
      echo "vopk: Packages to install (OpenBSD pkg_add):"
      printf '  %s\n' "${mapped_ob[@]}"
    ...
    netbsd)
      ...
      echo "vopk: Packages to install (NetBSD pkgin):"
      printf '  %s\n' "${mapped_nb[@]}"
    ...
    netbsd_pkg_add)
      ...
      echo "vopk: Packages to install (NetBSD pkg_add):"
      printf '  %s\n' "${mapped_np[@]}"
